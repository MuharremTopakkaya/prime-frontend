...
import HandleIcon from "images/handle-icon.svg?react";
import ArrowRightIcon from "images/arrow-right-3-icon.svg?react";
...
const UpdateNoticeIcon = styled(RadioIcon)`
  ${tw`w-0 sm:w-5 sm:mr-3`}
`;
...
const PreviewCardImage = styled(motion.div)<{ $imageSrc: string }>`
...
const StyledHandleIcon = styled(HandleIcon)`
  ${tw`w-4 h-4 text-gray-600`}
`;
const StyledArrowRightIcon = styled(ArrowRightIcon)`
  ${tw`transition duration-300 transform group-hover:translate-x-px ml-2 w-4 h-4`}
`;
...
interface MainLandingPageProps {
  ...
  buttonRoundedCss?: any;
  landingPages?: typeof components.landingPages;
  innerPages?: typeof components.innerPages;
  blocks?: typeof components.blocks;
  heading?: string;
  description?: string;
}
                ...
                <PreviewCardContainer key={index}>
                  <PreviewCard initial="rest" animate="rest" whileHover="hover" href={page.url} target="_blank">
                    <PreviewCardImageContainer>
                      <PreviewCardImage
                        transition={{ type: "tween" }}
                        variants={!page.scrollAnimationDisabled ? previewImageAnimationVariants : undefined}
                        $imageSrc={page.imageSrc}
                      />
                    </PreviewCardImageContainer>
                    <PreviewButton>View Live Demo</PreviewButton>
...
interface BlockProps {
  ...
  components: typeof components.blocks[keyof typeof components.blocks];
}

const Block: React.FC<BlockProps> = ({ notifyIsVisible, components }) => {
  ...
  const componentBlockRefs: Record<string, HTMLIFrameElement | null> = {};

  const updateComponentBlockIframeHeight = (iframe: HTMLIFrameElement | null) => {
            ...
            <ComponentContent>
              ...
              >
                <iframe
                  ...
                  onLoad={(e) => updateComponentBlockIframeHeight(e.target as HTMLIFrameElement)}
                />
              </ResizableBox>
            </ComponentContent>
          </Component>
